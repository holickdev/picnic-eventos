...
export function ImageGallery({ images, title, referencial_images }: ImageGalleryProps) {
    ...
    const handleArrowLeftKeyIfPressed = (event: KeyboardEvent) => {
        if (event.key === 'ArrowLeft') {
            prevImage();
        }
    };
    const handleArrowRightKeyIfPressed = (event: KeyboardEvent) => {
        if (event.key === 'ArrowRight') {
            nextImage();
        }
    };
    const handleEscapeKeyIfPressed = (event: KeyboardEvent) => {
        if (event.key === 'Escape') {
            setFullScreen(false);
        }
    };
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            handleArrowLeftKeyIfPressed(event);
            handleArrowRightKeyIfPressed(event);
            handleEscapeKeyIfPressed(event);
        };
    ...
    return (
        <motion.div
            ...
            className={`w-full h-full ${fullScreen ? 'fixed inset-0 z-100 bg-white' : ''}`}
        >
            <button
                onClick={() => setFullScreen(false)}
                className={`absolute top-4 right-4 z-[100] w-10 h-10 rounded-full bg-white/20 hover:bg-white/50 flex items-center justify-center transition-colors ${fullScreen ? 'block' : 'hidden'}`}
            >
